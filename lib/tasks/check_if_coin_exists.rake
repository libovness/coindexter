task :check_if_coin_exists => :environment do
	...
	coins.each do |coin| 
		if Coin.friendly.exists? coin[:name].downcase 
			c = Coin.friendly.find(coin[:name])
			add_coin_fields(c)
			n = Network.friendly.find(coin[:name])
			c.network = n
			c.save
		end
	end

	def add_network_fields(n)
	...
	def add_coin_fields(c)
		...
		unless coin[:ico_type_of_min_cap].nil?
			c.ico_type_of_min_cap = coin[:ico_type_of_min_cap]
		end
